...
z1 = equations.sol_affine(t,y0)
# Calcul de l'erreur maximum relative
e3 = np.max(np.abs((z1-y1)/z1))

N,h4 = 50,0.025
[t,y1] = methodes.euler_explicite(t0,h4,N,y0,equations.f_affine)
# Solution exacte aux mêmes instants
z1 = equations.sol_affine(t,y0)
# Calcul de l'erreur maximum relative
e4 = np.max(np.abs((z1-y1)/z1))

N,h5 = 50,0.0125
[t,y1] = methodes.euler_explicite(t0,h5,N,y0,equations.f_affine)
# Solution exacte aux mêmes instants
z1 = equations.sol_affine(t,y0)
# Calcul de l'erreur maximum relative
e5 = np.max(np.abs((z1-y1)/z1))

N,h6 = 50,0.00625
[t,y1] = methodes.euler_explicite(t0,h6,N,y0,equations.f_affine)
# Solution exacte aux mêmes instants
z1 = equations.sol_affine(t,y0)
# Calcul de l'erreur maximum relative
e6 = np.max(np.abs((z1-y1)/z1))
